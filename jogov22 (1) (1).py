...
class Jogador:
    ...
    def mostrar_vidas(self):
        print(f"{self.nome} tem {self.vidas} vidas!")
...
class Personagem:
    ...
    def __init__(self, nome, dano, vidas, nivel):
        ...
        self.__perguntas = self.carrega_perguntas(nivel)
        self.__ultima_resposta = ""
        ...
        perguntas = {
            ...
            "medio": [
                ...
                },
                {
                    ...
                    "respostas": {
                        "A": "Método que modifica o comportamento do operador para que ele atue como um tipo definido pelo programador",
    ...
    def obtem_pergunta(self):
        ...
        pergunta = choice(self.__perguntas)
        resposta_correta = pergunta["respostas"][pergunta["resposta_correta"]]

        # Embaralha as respostas para cada vez ter uma ordem diferente.
        respostas_embaralhadas = list(pergunta["respostas"].values())
        shuffle(respostas_embaralhadas)

        for i in range(len(pergunta["respostas"])):
            letra = list(pergunta["respostas"].keys())[i]
            pergunta["respostas"][letra] = respostas_embaralhadas[i]

            # Salva a nova letra da resposta correta
            if respostas_embaralhadas[i] == resposta_correta:
                pergunta["resposta_correta"] = letra

        return pergunta

    def define_resposta(self, letra):
        self.__ultima_resposta = letra
    ...
    def obtem_resposta(self, pergunta, respostas):
        ...
        botão_a = Button(janela, text=respostas["A"], command=lambda: [
                         self.define_resposta("A"),
                         janela.destroy()])

        botão_b = Button(janela, text=respostas["B"], command=lambda: [
                         self.define_resposta("B"),
                         janela.destroy()])

        botão_c = Button(janela, text=respostas["C"], command=lambda: [
                         self.define_resposta("C"),
                         janela.destroy()])
...
while True:
    ...
    resposta = input(">S/N ").upper().strip()
...
# Primeira pergunta de batalha do jogo
